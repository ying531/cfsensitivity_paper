...
if (pos == 1){
  ...
  write.csv(all.test, paste(out_dir, "sens_positive_mgn_hgamma_seed_",seed,".csv",sep=''))
}else{
  org.data$Y = -org.data$Y
  all.test = cbind(test.data, hat.gamma)
  write.csv(all.test, paste(out_dir, "sens_negative_mgn_hgamma_seed_",seed,".csv",sep=''))
}
